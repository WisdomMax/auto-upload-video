...
async def test_keyboard_reply():
    ...
    async with async_playwright() as p:
        ...
        print(f"2. @{target_username} 님 답글 달기 버튼 클릭...")
        ...
        # 입력창 클릭 후 keyboard.type으로 추가 메시지만 보충 (태그 보존!)
        input_box = page.locator("textarea, div[role='textbox']").first
        if await input_box.is_visible():
            await input_box.click()
            await asyncio.sleep(0.5)
            # keyboard.type 사용 (기존 @username 태그 뒤에 덧붙이기!)
            await page.keyboard.type(" 안녕하세요! DM으로 링크 보내드렸습니다 💕")
            ...
            if await post_btn.is_visible():
                ...
                await asyncio.sleep(3)
                print(f"✅ @{target_username} 태그 보존 대댓글 작성 완료!")

        await page.screenshot(path="/Volumes/NVME/7.AI_vibe_coding/20260605 momdad fashion diary/scratch/ig_preserve_tag_result.png")
        await context.close()

if __name__ == "__main__":
    asyncio.run(test_keyboard_reply())
